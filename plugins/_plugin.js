...
inrl.addCommand({pattern: 'install ?(.*)', fromMe: true, desc: Lang.INSTALL_DESC, warn: Lang.WARN, dontAddCommandList: true}, (async (message, match) => {
    ...
    if (response.statusCode == 200) {
        ...
        if (plugin_name.length >= 1) {
            plugin_name = "faz_" + plugin_name[1];
        } else {
            plugin_name = "faz_" + Math.random().toString(36).substring(8);
        }
...
inrl.addCommand({pattern: 'plugin$', fromMe: true, dontAddCommandList: true, desc: Lang.PLUGIN_DESC}, (async (message, match) => {
