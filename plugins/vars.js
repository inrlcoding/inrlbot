...
if (config.STANDPLK == 'off' || config.STANDPLK == 'OFF') {
   ...
   var l_dss = ''
    ...
    inrl.addCommand({pattern: 'theri ?(.*)', fromMe: true, desc: l_dss, usage: 'for pm .theri pm no / pm yes \n for group .theri gp no / gp yes' }, (async (message, match) => {
        if (match[1] == 'pm yes') {
                await heroku.patch(baseURI + '/config-vars', { 
                    body: { 
                        ['THERI_KICK_PM']: 'false'
 ...
 inrl.addCommand({pattern: 'bgm ?(.*)', fromMe: true, desc: plk_desc, usage: '.bgm one / two' }, (async (message, match) => {
        if (match[1] == 'two') {
                await heroku.patch(baseURI + '/config-vars', { 
                    body: { 
                        ['CHANGE_BGM_TO']: 'two'
   ...
   var l_dsc = ''
    ...
    inrl.addCommand({pattern: 'antilink ?(.*)', fromMe: true, desc: l_dsc, usage: '.antilink on / off' }, (async (message, match) => {
        if (match[1] == 'off') {
                await heroku.patch(baseURI + '/config-vars', { 
                    body: { 
                        ['ANTİ_LİNK']: 'false'
   ...
   var l_dsc = ''
    ...
    inrl.addCommand({pattern: 'mlink ?(.*)', fromMe: true, desc: l_dsc, usage: '.mlink on / of' }, (async (message, match) => {
        if (match[1] == 'off') {
                await heroku.patch(baseURI + '/config-vars', { 
                    body: { 
                        ['ALL_LINK_BAN']: 'false'
 ...
 inrl.addCommand({pattern: 'austick ?(.*)', fromMe: true, desc: plk_desc, usage: '.austick on / off' }, (async (message, match) => {
        if (match[1] == 'on') {
                await heroku.patch(baseURI + '/config-vars', { 
                    body: { 
                        ['STICKER_REPLY']: 'true'
 ...
 var W_ADM = ''
  if (config.LANG == 'EN') {
    ...
    inrl.addCommand({pattern: 'work ?(.*)', fromMe: true,dontAddCommandList: true, }, (async (message, match) => {
        if (match[1] == 'public') {
                await heroku.patch(baseURI + '/config-vars', { 
                    body: { 
                        ['WORK_TYPE']: 'public'
...
inrl.addCommand({pattern: 'standby ?(.*)', fromMe: true, desc: plk_STN }, (async (message, match) => {
                await heroku.patch(baseURI + '/config-vars', { 
                    body: { 
                        ['STANDBY_MODE']: 'on'
                    } 
...
inrl.addCommand({pattern: 'run again ?(.*)', fromMe: true, desc: AFN_STN }, (async (message, match) => {
                await heroku.patch(baseURI + '/config-vars', { 
                    body: { 
                        ['STANDBY_MODE']: 'off'
                    } 
...
var PRE_dsc = ''
    ...
    inrl.addCommand({pattern: 'presence ?(.*)', fromMe: true, desc: PRE_dsc, usage: '.presence online / offline / typing / recording' }, (async (message, match) => {
    ...
    inrl.addCommand({ pattern: 'duration ?(.*)', fromMe: true, desc: 'changes bgm duration', usage: '.bgmduration *9999999*' }, (async (message, match) => {
